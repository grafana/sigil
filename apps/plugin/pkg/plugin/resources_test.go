	...
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/datasources/proxy/uid/tempo/api/search":
			...
			if strings.TrimSpace(r.URL.Query().Get("q")) != "" {
				...
				_, _ = io.WriteString(w, `{"traces":[{"traceID":"trace-1","startTimeUnixNano":"1739612400000000000","spanSets":[{"spans":[{"spanID":"span-1","durationNanos":"1000000000","attributes":[{"key":"sigil.generation.id","value":{"stringValue":"gen-1"}},{"key":"gen_ai.conversation.id","value":{"stringValue":"conv-1"}},{"key":"gen_ai.request.model","value":{"stringValue":"gpt-4o"}},{"key":"gen_ai.agent.name","value":{"stringValue":"assistant"}},{"key":"sigil.user.name","value":{"stringValue":"Taylor"}}]}]}]}]}`)
				return
			}
