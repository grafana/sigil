...
describe('useConversationData', () => {
  ...
  it('preserves trace-enriched spans when zero-progress pagination stabilizes', async () => {
    let resolveStalledPage: ((value: unknown) => void) | undefined;
    let resolveInitialTrace: ((value: unknown) => void) | undefined;
    const dataSource: ConversationsDataSource = {
      searchConversations: jest.fn(),
      getConversationDetail: jest
    ...
    await act(async () => {
      loadMorePromise = result.current.loadMoreGenerations();
      await Promise.resolve();
    });

    await waitFor(() => {
      expect(result.current.loadingMoreGenerations).toBe(true);
    });
