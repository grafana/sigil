...
BUGBOT_REVIEW_MARKERS = (
    "<!-- BUGBOT_REVIEW -->",
    "Cursor Bugbot has reviewed your changes",
    "Bugbot Autofix prepared",
)
MAX_GH_RETRIES = 5
...
def is_bugbot_body(body: str) -> bool:
    return any(marker in body for marker in BUGBOT_REVIEW_MARKERS)
...
def filter_bugbot_issue_comments(
    comments: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    latest_ack = latest_codex_issue_reply_time(comments)
    filtered: list[dict[str, Any]] = []
...
def filter_bugbot_review_comments(
    comments: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    latest_codex_reply = latest_codex_reply_by_thread(comments)
    filtered: list[dict[str, Any]] = []
...
def raise_on_human_feedback(
    ...
    bugbot_issue_comments = filter_bugbot_issue_comments(issue_comments)
    bugbot_review_comments = filter_bugbot_review_comments(review_comments)
    bugbot_reviews = filter_bugbot_reviews(reviews, head_sha)
    if bugbot_issue_comments or bugbot_review_comments or bugbot_reviews:
        print("Bugbot found issues on the current PR head. Fix them before merge.")
        print(
            "Acknowledge the feedback with a [codex] comment, push fixes, and "
            "wait for Bugbot to rerun cleanly on the latest head.",
        )
        raise SystemExit(2)
