...
- Treat Cursor Bugbot feedback as blocking on the current PR head: fix the
  surfaced issue, push, and wait for a clean rerun before merging.
...
6. Ensure Codex review comments and Cursor Bugbot findings (if present) are
...
9. If Bugbot leaves a review or issue comment on the current PR head, treat it
   as blocking correctness feedback even if GitHub checks are green:
   - reply with `[codex]` describing whether you will fix or defer it,
   - implement the fix when accepted,
   - commit + push,
   - wait for Bugbot to rerun on the new head,
   - do not merge until the latest Bugbot pass leaves no unresolved findings.
...
Preferred: use the asyncio watcher to monitor review comments, Bugbot findings,
...
- 2: Review comments or Bugbot findings detected (address feedback)
- 3: CI checks failed
...
- If checks fail, pull details with `gh pr checks` and `gh run view --log`, then
  ...
  re-run the watch.
- If Bugbot comments on the current head, treat it as blocking feedback even if
  the check suite is otherwise green. Fix the issue, push, and rerun the watch
  until Bugbot leaves no new findings on the latest head.
...
- Do not merge while current-head Bugbot findings are outstanding.
...
- Cursor Bugbot feedback arrives from `cursor` as a review or issue comment and
  includes markers such as `<!-- BUGBOT_REVIEW -->` or `Bugbot Autofix`.
  Treat Bugbot findings on the current PR head as blocking until you either fix
  them or explicitly defer with rationale.
...
- For Bugbot review comments or issue comments, reply with `[codex]` before
  changing code, then post a follow-up `[codex]` comment after the fix with the
  commit sha and validation summary.
...
- Only request a new Codex review when you need a rerun (e.g., after new
  ...
  - If Bugbot was part of the previous feedback batch, also wait for Bugbot to
    rerun on the latest head and confirm it leaves no new findings before
    merging.
