...
func TestGetConversationDetailPageForTenantEmptyPageDoesNotLoopCursor(t *testing.T) {
	base := time.Date(2026, 2, 15, 9, 0, 0, 0, time.UTC)
	conversationStore := &stubConversationStore{
		items: map[string]storage.Conversation{
			"conv-1": {
	...
	if detail.HasMore {
		t.Fatalf("expected has_more=false when page returns zero results despite higher GenerationCount")
	}
	if detail.NextCursor != "" {
		t.Fatalf("expected empty next cursor when page is empty, got %q", detail.NextCursor)
	}
	if len(detail.Generations) != 0 {
		t.Fatalf("expected 0 generations on empty page, got %d", len(detail.Generations))
	}
}
